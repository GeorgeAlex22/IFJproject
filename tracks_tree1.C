...
void tracks_tree::Loop()
{
   ...
   TH2D *h_dEdxVSp_pos_before = new TH2D("h_dEdxVSp_pos_before", "dE/dx vs p", 100, -0.5, 2.3, 100, 0.5, 2);
   TH2D *h_dEdxVSp_neg_before = new TH2D("h_dEdxVSp_neg_before", "dE/dx vs p", 100, 0, 0, 100, 0.5, 2);

   TH2D *h_dEdxVSp_pos_after = new TH2D("h_dEdxVSp_pos_after", "dE/dx vs p", 100, 0.6, 2.1, 100, 0.5, 2);
   TH2D *h_dEdxVSp_neg_after = new TH2D("h_dEdxVSp_neg_after", "dE/dx vs p", 100, 0, 0, 100, 0.5, 2);

   TH2D *h_dEdxVSp_pos_probcut = new TH2D("h_dEdxVSp_pos_probcut", "dE/dx vs p", 100, 0.6, 2.1, 100, 0.5, 2);
   TH2D *h_dEdxVSp_neg_probcut = new TH2D("h_dEdxVSp_neg_probcut", "dE/dx vs p", 100, 0, 0, 100, 0.5, 2);
   ...
   for (Long64_t jentry = 0; jentry < nentries; jentry++)
   {
      ...
      if (vertex_cut)
      {
         ...
         if (npratio_cut && bxby_cut)
         {
            ...
            tmp_id_number = Event_id;
            nb = fChain->GetEntry(jentry + 1);
            bool same_event = (tmp_Run_number == Run_number) && (tmp_id_number == Event_id);
