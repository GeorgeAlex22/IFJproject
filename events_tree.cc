...
void Loop(TChain &fChain, char* outputFileName, bool Debug)
{
   ...
   for (Long64_t jentry = 0; jentry < nentries; jentry++)
   {
      ...
      for (const auto& t: pevent->tracks) {
          ...
          if (! (npratio_cut && bxby_cut)) continue;
          ...
          if (!(log10(p) >= 0.6 && log10(p) <= 2.1)) continue;
          if (!(abs(t.dEdx) >= 0.5 && 
                abs(t.dEdx) <= dEdx_BBp + 0.15 * (dEdx_BBk - dEdx_BBp))) continue;
...
int main(int argc, char **argv)
{
   ...
   {
      if (! strncmp(argv[i],"-d",2) ) { // check if we have
